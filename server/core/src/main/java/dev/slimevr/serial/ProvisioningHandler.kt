...
class ProvisioningHandler(private val vrServer: VRServer) : SerialListener {
	...
	private val listeners: MutableList<ProvisioningListener> = CopyOnWriteArrayList()
	...
	private val MAX_CONNECTION_RETRIES: Byte = 1

	init {
		vrServer.serialHandler.addListener(this)
		this.provisioningTickTimer.scheduleAtFixedRate(
			object : TimerTask() {
				override fun run() {
					if (!isRunning) return
					provisioningTick()
	...
	fun provisioningTick() {
		...
		if (this.provisioningStatus == ProvisioningStatus.SERIAL_INIT &&
			vrServer.serialHandler.knownPorts.findAny()
				.isEmpty &&
			...
			this.listeners
				.forEach(
					Consumer { l: ProvisioningListener ->
						l
							.onProvisioningStatusChange(
								status,
								vrServer.serialHandler.getCurrentPort()!!,
							)
					},
				)
